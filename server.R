...
shinyServer(function(input, output, session) {  #Begin shiny server portion of app
  ...
  plotInput <- reactive({
    ...
    if(input$dataset==3){ ##Blame DV if this subset code doesnt work
      if(input$bc_subtype!="all_breast_carcinoma"){
        subtype<-read.delim("BRCA_PAM50subtype.txt")
        data2<-datasetInput()
        pts.df<-as.character(data2[,1])
        pts.subtype<-as.character(subtype$sample)
        subtype_class<-subtype[match(pts.df, pts.subtype),2]
        data2<-data.frame(data2, subtype_class)
        subset<-subset(data2, subtype_class==input$bc_subtype)
        data2<-subset
  ...
  dataInput2 <- reactive({
    ...
    if(input$dataset2==3){ ##Blame DV if this subset code doesnt work
      if(input$bc_subtype2!="all_breast_carcinoma"){
        subtype<-read.delim("BRCA_PAM50subtype.txt")
        pts.df<-as.character(data3[,1])
        pts.subtype<-as.character(subtype$sample)
        subtype_class<-subtype[match(pts.df, pts.subtype),2]
        data3<-data.frame(data3, subtype_class)
        subset<-subset(data3, subtype_class==input$bc_subtype2)
        data3<-subset
    ...
    GGresults2 <- GGresults %>%
      ...
      mutate(HR_inverse = 1/as.numeric(HR))
    rownames(GGresults2) <- names
    GGresults2$names <- row.names(GGresults2)
    GGresults2 <- subset(GGresults2, log10P > 0.1 & HR <= 20)
    
    if (input$log2 == "-4") {
      GGresults2$HR_inverse <- log(GGresults2$HR_inverse)
    }
    
    data4 <- GGresults2 %>%  #Filter which data points to plot, 1.3 == -log10(0.05)
      filter(log10P >= 1.3 & as.numeric(HR_inverse) >= input$adjusthigh | log10P >= 1.3 & as.numeric(HR_inverse) <= input$adjustlow)
    data4 <- data4 %>%
      mutate(CAT = ifelse(HR_inverse > 1, "Poor", "Good"))  #Mutate adds a column with variable Poor or Good which is dependent on the Hazard Ratio, high-vs-low
  ...
  plotInput2 <- reactive({
    data3 <- Input2()
    names <- t(colnames(data3)[c(-(1:5))])
    
    dataset<-dataInput2()
      if (input$log2 == "1") {
        if (input$prop == "1") {  #Creation of volcano plot with dynamic labeling based on user selected ranges
        res2 <- ggplot(subset(dataset, log10P > 0.1), aes(y=log10P, x=as.numeric(HR_inverse))) + 
          geom_jitter(width = 0.05, shape=21, alpha=0.8, aes(size=log10P, fill=as.numeric(HR_inverse))) + 
          geom_hline(yintercept = 1.3, lty=2) +
          geom_vline(xintercept = 1) +
          geom_label_repel(data=subset(dataset, log10P >= 1.3 & as.numeric(HR_inverse) >= input$adjusthigh | log10P >= 1.3 & as.numeric(HR_inverse) <= input$adjustlow),
